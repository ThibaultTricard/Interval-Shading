...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(1920, 1080, "Interval Shading - Bunny", nullptr, nullptr);
	...
	Tetmesh mesh = load_msh(root+"/objects/bunny.vtk");
	...
	MeshShaderModule meshShader(root+"/shaders/bunny/mesh.mesh.spv");
	FragmentShaderModule fragmentShader(root+"/shaders/bunny/interval.frag.spv");
	

	VkExtent2D size = swapChain->size();
	
	Trackball trackball(vec3f({0.0f,-1.1,0.0f}),vec3f({0.0f,0.0f,0.0f}),vec3f({0.0f,0.0f,-1.0f}));
    ...
    VertexShaderModule possProcessVert(root+"/shaders/bunny/postProcess.vert.spv");
	FragmentShaderModule possProcessFrag(root+"/shaders/bunny/postProcess.frag.spv");
