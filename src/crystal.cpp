...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(1920, 1080, "Interval Shading - Crystal", nullptr, nullptr);
	...
	Tetmesh mesh = load_msh(root+"/objects/crystal.vtk");
	...
	MeshShaderModule meshShader(root+"/shaders/crystal/mesh.spv");
	FragmentShaderModule fragmentShader(root+"/shaders/crystal/interval.spv");
	...
	Trackball trackball(vec3f({0.0f,-0.8f,0.0f}),vec3f({0.0f,0.0f,0.0f}),vec3f({0.0f,0.0f,-1.0f}));
	trackball.m_moveSpeed = 0.001f;
	...
	auto envmap = createTextureBuffer(graphicQueue,commandBuffer,root+"envmaps/dancing_hall_4k.png",4);

	vec4f color;
	vec4f lightColor;
	color = vec4f({255/255.0,174/255.0,26/255.0f,17.8f});
	vec4f reflection = vec4f({1.0,1.0,1.0f,0.3});
	...
	postProcessDescriptor.addTextureBuffer(envmap,VK_SHADER_STAGE_FRAGMENT_BIT,2);
    ...
    VertexShaderModule possProcessVert(root+"/shaders/crystal/postProcess.vert.spv");
	FragmentShaderModule possProcessFrag(root+"/shaders/crystal/postProcess.frag.spv");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		ImGui::SliderInt("Mode: ", &step, 0,3);
		ImGui::Text(text.data());
		ImGui::ColorEdit3("color", &color.data[0]);
		ImGui::SliderFloat("Opacity", &color.data[3],0,100.0);
		ImGui::ColorEdit3("Reflection color", &reflection.data[0]);
		ImGui::SliderFloat("reflectivity", &reflection.data[3],0,1);
		ImGui::End();
		...
		viewproj.setVariable("mode", step);
